...
local function updateMembers(gangName)
    ...
    for _, playerId in ipairs(GetPlayers()) do
        TriggerEvent("GangSystem:server:GetGangStuff", playerId, gangName)
    end
end
...
RegisterNetEvent("GangSystem:server:GetGangStuff")
AddEventHandler("GangSystem:server:GetGangStuff", function(source, name)
    local result = exports.oxmysql:executeSync('SELECT * FROM gangs WHERE name=@GangName', {
        ['@GangName'] = name,
    })
    if result[1] then
        ...
        if members[license] then
            ...
            TriggerClientEvent("GangSystem:client:GetGangStuff", source, gangData, license)
        end
    end
end)
